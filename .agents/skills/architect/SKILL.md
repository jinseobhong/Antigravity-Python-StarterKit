...
name: architect
description: >-
  Phase 1 Architecture & Design skill for Antigravity (아키텍처 설계 단계). Use when the user requests a new feature,
  system architecture design, or task planning without immediate coding. Enforces CDD contracts, constitution sync, context restoration, implementation_plan drafting, and mandatory twin-call plan mirroring.
---
...
# Architecture & Design Runbook (Phase 1: 아키텍처 설계 단계)
...
이 스킬은 작업을 시작하기 전 **1단계: 방향 및 요구 파악, CDD API 규격 동결, 시스템 아키텍처 설계, 구현 계획서 수립, `docs/architecture/` 동기화**를 수행하는 전용 런북이다.

---

## 🛠️ 필수 수행 절차 (Mandatory Steps)

1. **현재 상황 파악 (Step 1)**:
   - 무권대리 금지 및 사전 승인 원칙을 상기하고, `<ProjectRoot>/docs/architecture/CURRENT_STATE.md`와 `docs/architecture/DEVELOPMENT_GUIDE.md`를 조회하여 API 계약을 복원한다.
2. **요구사항 및 핵심 핀포인트 정적 분석 (Step 2)**:
   - 사용자 의도를 확인하고 대상 파일, 호출 의존성, 5대 REST API 계약을 정적으로 탐색한다.
3. **구현 계획서 수립 및 쌍둥이 호출 미러링 (Step 3 — Twin-Call Mandate)**:
   - 세션 브레인에 `implementation_plan.md` 아티팩트를 작성(`RequestFeedback: true`)할 때, **반드시 동일한 턴에서 `<ProjectRoot>/docs/architecture/IMPLEMENTATION_PLAN.md`에도 `write_to_file` 도구를 호출하여 100% 동일한 내용을 즉시 동기화(Twin-Call)**한다.
4. **진척 현황판 갱신 (Step 4)**:
   - `<ProjectRoot>/docs/architecture/IMPLEMENTATION_STATUS.md` 대상 모듈 상태를 `WIP`로 등록한다.
   - `<ProjectRoot>/docs/architecture/CURRENT_STATE.md` 파이프라인 좌표를 `[승인 대기]`로 업데이트한다.

---

## 🛑 HARD STOP (인간 사전 검토 및 승인 대기)
- **계획서 제출 직후 도구 호출을 즉시 종료하고 인간 사용자의 명시적 사전 승인을 대기한다.**
- 인간의 명시적 사전 승인 없이 소스코드를 수정하는 일체의 행위는 엄격히 금지된다.
