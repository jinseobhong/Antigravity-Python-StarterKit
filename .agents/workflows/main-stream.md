---
description: >-
  Master Intent Classifier & Track Router (/main-stream). Dynamically classifies requests into 4 tracks and enforces CDD & E2E verification.
---
...
# Master Intent Classifier & Track Router (`/main-stream`)
...
이 워크플로우는 사용자의 모든 요청을 분석하여 **4대 트랙 중 최적의 경로로 동적 라우팅**하는 마스터 제어기입니다.

---

## 🚦 4대 트랙 분류 기준

1. **Track 1: Quick Track (패스트 트랙)**: 단순 오타, 주석, 단일 파일 경미한 수정 ➔ 즉시 수정 후 diff 보고
2. **Track 2: Spike Track (기술 검증)**: 일회성 프로토타입/PoC ➔ 스파이크 브랜치/격리 실행
3. **Track 3: Standard Architecture (표준 2단계 아키텍처 트랙)**: 신규 기능, 구조 변경, API 변경 ➔ `/architect` (승인 대기) ➔ `/implement` (E2E 실측 검증)
4. **Track 4: General / Conversational Track**: 단순 질의응답 및 토론 ➔ 직접 답변
