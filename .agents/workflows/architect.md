---
description: >-
  Phase 1 Architecture & Design Workflow (/architect - 아키텍처 설계 단계). Restores context, analyzes scope, enforces CDD & E2E contracts, and drafts views/IMPLEMENTATION_PLAN.md.
---
...
# Architecture & Design Workflow (`/architect`)
...
이 워크플로우는 새로운 기능/작업 착수 시 **1단계 아키텍처 설계(Architect), CDD API 계약 동결, 구현 계획서 수립, 승인 대기**를 진행하는 대화형 리모컨입니다.

---

## 📌 아키텍처 설계 단계 실행 체크리스트

1. **상태 복원**: `<ProjectRoot>/docs/architecture/CURRENT_STATE.md` 확인
2. **API Contract 검토**: `docs/architecture/DEVELOPMENT_GUIDE.md` 5대 REST API 규격 준수 여부 정적 분석
3. **설계 수립 & 미러링**: 
   - 세션 브레인 `implementation_plan.md` 작성
   - `<ProjectRoot>/docs/architecture/IMPLEMENTATION_PLAN.md`로 자동 복사 동기화
4. **진척 갱신**: 
   - `docs/architecture/IMPLEMENTATION_STATUS.md` 대상 모듈 `[WIP]` 등록
   - `docs/architecture/CURRENT_STATE.md` `[승인 대기]` 갱신
5. 🛑 **강제 정지 (HARD STOP)**: 사용자의 승인이 있을 때까지 코드 수정을 일절 진행하지 않고 대기.
